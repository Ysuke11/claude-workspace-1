...
分析・調査・データの報告では**事実と仮説を必ず分離して記載する**。
詳細フォーマットは `.claude/skills/fact-hypothesis-report/SKILL.md`（要点:
事実=計算方法を明示できる数値のみ・定義併記、仮説=確度と検証方法をセットで書く。
因果の主張・「依存」「原因」等の解釈語・運用状態の推測は必ず仮説側に置く）。
